...
async function LoadComponentVariation(ped, component, drawable, texture) {
	...
	if (component === 2) { 
		SetPedComponentVariation(ped, 2, drawable, 1, 0);
	} else { 
...
async function InitializePedComponents() {
    if (config.debug) console.log(`DEBUG: Initializing Ped Components`);

    // Configuration de base du personnage
	SetPedDefaultComponentVariation(ped);
    await Delay(150);


    // Initialisation avec une tenue de base complète
    const defaultComponents = {
        ...
        6: [1, 0],     // Shoes
        7: [0, 0],     // Accessories
        8: [15, 0],    // Undershirt - t-shirt basique
        9: [0, 0],     // Body Armor
        10: [0, 0],    // Decals
        11: [0, 0],    // Top/Jacket
    };

    // Application des composants de base
    for (const [component, [drawable, texture]] of Object.entries(defaultComponents)) {
        await LoadComponentVariation(ped, parseInt(component), drawable, texture);
    }

    // S'assurer que le personnage a des vêtements de base minimaux
    await LoadComponentVariation(ped, 3, 15, 0);  // Bras standards
    await LoadComponentVariation(ped, 4, 14, 1);  // Pantalon standard
    await LoadComponentVariation(ped, 6, 1, 0);   // Chaussures standards
    await LoadComponentVariation(ped, 8, 15, 0);  // T-shirt standard
    await LoadComponentVariation(ped, 11, 15, 0); // Haut standard
	await LoadComponentVariation(ped, 6, 34, 0); // Chassure 


    return;
}

// Nouvelle fonction pour mettre à jour un composant spécifique
async function UpdatePedComponent(component, drawable, texture = 0, type = 'CLOTHING') {
    if (config.debug) console.log(`DEBUG: Updating Component: ${type} ${component} ${drawable} ${texture}`);
    ...
    if (type === 'CLOTHING') {
        ...
        SetPedHeadOverlay(ped, component, drawable, 1.0);
        if (texture !== undefined) {
            SetPedHeadOverlayColor(ped, component, 1, texture, 0);
        }
    }
}
...
RegisterCommand('screenshot', async (source, args) => {
    ...
    for (const modelHash of modelHashes) {
        if (IsModelValid(modelHash)) {
            ...
            // Parcourir et mettre à jour les composants
            for (const type of Object.keys(config.cameraSettings)) {
                for (const stringComponent of Object.keys(config.cameraSettings[type])) {
                    ...
                    // Réinitialiser le composant précédent à 0
                    if (lastComponent !== null && lastType !== null) {
                        if (lastType === 'CLOTHING') {
                            await LoadComponentVariation(ped, lastComponent, 0, 0);
                        } else if (lastType === 'PROPS') {
                            await LoadPropVariation(ped, lastComponent, 0, 0);
                        } else if (lastType === 'TKT') {
                            SetPedHeadOverlay(ped, lastComponent, 0, 0.0);
                        }
                    }
                    ...
                    if (type === 'CLOTHING') {
                        ...
                        for (let drawable = 0; drawable < drawableVariationCount; drawable++) {
                            ...
                            await UpdatePedComponent(component, drawable, 0, type);
                            ...
                            if (config.includeTextures) {
                                ...
                                for (let texture = 0; texture < textureVariationCount; texture++) {
                                    await UpdatePedComponent(component, drawable, texture, type);
                        ...
                        for (let prop = 0; prop < propVariationCount; prop++) {
                            ...
                            await UpdatePedComponent(component, prop, 0, type);
                            ...
                            if (config.includeTextures) {
                                ...
                                for (let texture = 0; texture < textureVariationCount; texture++) {
                                    await UpdatePedComponent(component, prop, texture, type);
                        ...
                        const overlays = {
                            2: { name: 'Sourcils', maxValue: 33 },
                            1: { name: 'Barbe', maxValue: 28 },
                            4: { name: 'Maquillage', maxValue: 23 }
                        };
                    
                        const overlayIdNum = parseInt(component);
                        if (overlays[overlayIdNum]) {
                            const overlayInfo = overlays[overlayIdNum];
                            
                            for (let value = 0; value < overlayInfo.maxValue; value++) {
                                SendNUIMessage({
                                    type: overlayInfo.name,
                                    value: value,
                                    max: overlayInfo.maxValue
                                });
                    
                                await UpdatePedComponent(overlayIdNum, value, undefined, type);
                                await takeScreenshotForComponent(pedType, type, overlayIdNum, value);

                                if (config.includeTextures) {
                                    const colorCount = GetNumHairColors();
                                    for (let colorId = 0; colorId < colorCount; colorId++) {
                                        await UpdatePedComponent(overlayIdNum, value, colorId, type);
                                        await takeScreenshotForComponent(pedType, type, overlayIdNum, value, colorId);
                                    }
                                }
                            }
                        }
                    ...
                    // Sauvegarder le composant actuel comme dernier composant
